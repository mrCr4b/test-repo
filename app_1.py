...
def run_app():
    ...
    # Cập nhật màu sắc chủ đạo cho giao diện
    ...
    frame_user = tk.Frame(frame, bg="#f7f7f7")
    frame_user.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    tk.Label(frame_user, text="Chọn tệp người dùng", bg="#f7f7f7", font=("Arial", 12)).grid(row=0, column=0, sticky="w", padx=5)
    user_file = ttk.Entry(frame_user, width=50)
    ...
    ttk.Button(frame_user, text="Chọn tệp người dùng", command=browse_user_file).grid(row=0, column=2, padx=5)

    user_text_widget = tk.Text(frame_user, width=50, height=5)
    user_text_widget.grid(row=1, column=0, columnspan=3, pady=10)

    # Nút lưu
    ttk.Button(frame_user, text="Lưu tệp người dùng", command=lambda: save_to_file(user_file.get(), user_text_widget.get("1.0", tk.END))).grid(row=2, column=0, columnspan=3, pady=5)

    # Khung 2: Tệp từ khóa
    frame_keyword = tk.Frame(frame, bg="#f7f7f7")
    frame_keyword.grid(row=1, column=0, padx=5, pady=5, sticky="w")

    tk.Label(frame_keyword, text="Chọn tệp từ khóa", bg="#f7f7f7", font=("Arial", 12)).grid(row=0, column=0, sticky="w", padx=5)
    keyword_file = ttk.Entry(frame_keyword, width=50)
    ...
    ttk.Button(frame_keyword, text="Chọn tệp từ khóa", command=browse_keyword_file).grid(row=0, column=2, padx=5)

    keyword_text_widget = tk.Text(frame_keyword, width=50, height=5)
    keyword_text_widget.grid(row=1, column=0, columnspan=3, pady=10)

    # Nút lưu
    ttk.Button(frame_keyword, text="Lưu tệp từ khóa", command=lambda: save_to_file(keyword_file.get(), keyword_text_widget.get("1.0", tk.END))).grid(row=2, column=0, columnspan=3, pady=5)

    # Khung 3: Nhập email
    frame_email = tk.Frame(frame, bg="#f7f7f7")
    frame_email.grid(row=2, column=0, padx=5, pady=5, sticky="w")

    tk.Label(frame_email, text="Nhập email nhận thông báo", bg="#f7f7f7", font=("Arial", 12)).grid(row=0, column=0, pady=5, sticky="w")
    recipient_email = ttk.Entry(frame_email, width=50)
    recipient_email.grid(row=0, column=1, pady=5)

    # Khung 4: Nút quét và kết quả
    frame_scan = tk.Frame(frame, bg="#f7f7f7")
    frame_scan.grid(row=3, column=0, padx=5, pady=5, sticky="w")

    # Nút quét
    ttk.Button(frame_scan, text="Quét 1 lần", command=lambda: scan_once(user_file.get(), keyword_file.get(), log_text_widget, recipient_email.get())).grid(row=0, column=0, columnspan=3, pady=10)
